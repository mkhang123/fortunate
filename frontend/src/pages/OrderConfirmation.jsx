...
export default function OrderConfirmation() {
 ...
 const stored = JSON.parse(localStorage.getItem("guestOrderLookup") || "{}");
 return emailFromUrl && phoneFromUrl
 ...
 const effectiveLookup = getGuestLookup();
 if (
 ...
 let response;
 if (user) {
 response = await api.get(`/orders/${orderId}/invoice`, {
 responseType: "blob",
 });
 } else {
 const lookup = getGuestLookup();
 response = await api.get(`/orders/guest/${orderId}/invoice`, {
 params: { email: lookup.email, phone: lookup.phone },
 responseType: "blob",
 });
 }
 ...
 console.error("Lỗi tải hóa đơn PDF:", err);
 ...
 <span
 className={`inline-block px-4 py-2 rounded-full text-[10px] font-black tracking-wider border ${getStatusColor(
 order.status
 )}`}
 >
 ...
 <span
 className={`inline-block px-4 py-2 rounded-full text-[10px] font-black tracking-wider border ${order.payment.status === "SUCCESS"
 ...
 <Link
 to={user ? "/my-orders" : "/"}
 className="flex-1 bg-black text-white py-4 rounded-2xl text-[10px] font-black tracking-[0.2em] text-center hover:bg-gray-800 transition-all"
 >
 ...
 disabled={pdfLoading}
 className="flex-1 border-2 border-black text-black py-4 rounded-2xl text-[10px] font-black tracking-[0.2em] text-center hover:bg-black hover:text-white transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {pdfLoading ? (
 ...
 <Link
 to="/clothes"
 className="flex-1 bg-gray-100 text-black py-4 rounded-2xl text-[10px] font-black tracking-[0.2em] text-center hover:bg-gray-200 transition-all"
 >
 Tiếp tục mua sắm
 </Link>
 </div>
